...
import { ChevronLeft, Edit, Save, Star, Trash, CheckCircle, PlusCircle, Clock, Users } from "lucide-react"
...
const queueData = {
  ...
  description:
    "Queue for handling customer support requests and inquiries from customers needing assistance with our products and services.",
  createdAt: "2025-04-01T10:00:00",
  lastModified: "2025-04-07T14:30:00",
  isPublic: true,
}
...
export function QueueManagementPage() {
  ...
  // Save queue title
  const saveTitle = () => {
    setQueue({ ...queue, title: newTitle })
    setIsEditingTitle(false)
  }

  // Save queue description
  const saveDescription = () => {
    setQueue({ ...queue, description: newDescription })
  ...
  const togglePriority = (id: string) => {
    setPeople(people.map((person) => (person.id === id ? { ...person, isPriority: !person.isPriority } : person)))
  }
                ...
                <div className="space-y-1 flex-1">
                  {isEditingTitle ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {queue.isPublic && <Badge variant="outline">Public</Badge>}
                    </div>
                  )}
                  ...
                  {isEditingDescription ? (
                    ...
                    <div className="flex items-start gap-2">
                      <CardDescription className="flex-1">{queue.description}</CardDescription>
                      <Button variant="ghost" size="sm" className="mt-0" onClick={() => setIsEditingDescription(true)}>
                        <Edit className="h-4 w-4" />
            ...
            <CardContent>
              <div className="flex flex-wrap gap-6">
                <div className="flex items-center">
                  ...
                  <span>
                    <strong>{people.length}</strong> {people.length === 1 ? "person" : "people"} in queue
                  </span>
                </div>
                <div className="flex items-center">
                  <Star className="mr-2 h-4 w-4 text-muted-foreground" />
                  <span>
                    <strong>{people.filter((p) => p.isPriority).length}</strong> priority
                  </span>
                </div>
                <div className="flex items-center">
                  ...
                  <span>
                    ...
                    <strong>
                      {people.length > 0
                        ? Math.round(people.reduce((acc, p) => acc + calculateWaitTime(p.joinedAt), 0) / people.length)
                        : 0}{" "}
                      min
                    </strong>
                  </span>
            ...
            <Dialog open={isAddingPerson} onOpenChange={setIsAddingPerson}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Add a new person to the queue. They will be added to the end of the queue.
                  </DialogDescription>
                </DialogHeader>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            size="sm"
                            onClick={() => togglePriority(person.id)}
